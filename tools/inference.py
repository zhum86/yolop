...
class YOLOP:
    ...
    def detect(self, img_det, od_conf_thres=0.25, od_iou_thres=0.45):
        ...
        # Padded resize
        img, ratio, pad = letterbox_for_img(img_det, new_shape=self.img_size, auto=True)
        h, w = img.shape[:2]
        ...
        inf_out, _ = det_out
        # apply NMS
        det_pred = non_max_suppression(inf_out, conf_thres=od_conf_thres, iou_thres=od_iou_thres, classes=None, agnostic=False)
        det = det_pred[0]
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--img_height', type=int, default=640, help='inference size (pixels)')
    # parser.add_argument('--img_width', type=int, default=640, help='inference size (pixels)')
    ...
    with torch.no_grad():
        ...
        for img_fn in image_list:
            img0 = cv2.imread(img_fn, cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION)  # BGR
            yolop.detect(img0)